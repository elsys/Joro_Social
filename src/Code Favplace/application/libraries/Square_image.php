...
class Square_image
{
	...
	function resize_to_min ($originalFile, $newWidth = 120, $newHeight = 120, $newFile = '')
    {
		...
		$ratio = max($newWidth/$imgData['width'] , $newHeight/$imgData['height']);
		if($newWidth>$imgData['width'])
		{
			$newWidth = $ratio * $imgData['width'];
		}
		if($newHeight>$imgData['height']) $newHeight = $ratio * $imgData['height'];		
	...
	function rectangle($originalFile, $newFile = '', $w_width=200, $w_height=100, $offset=0)
	{		
		// Abort if image does not exist
        if (! file_exists($originalFile) || ! is_file($originalFile)) {
            return false;
        ...
        // Crop only if image is not square yet
        if ($imgData['width'] >= $w_width || $imgData['height'] >= $w_height) {
            // Set x and y axis for cropping. If x and y axis have not been 
            // passed as parameter we will crop from the center of the image.
			$config['width'] = $imgData['width'] > $w_width ? $w_width : $imgData['width'];
            $config['height'] = $imgData['height'] > $w_height ? $w_height : $imgData['height'];
            $config['x_axis'] = $offset > 0 ? $offset : ($imgData['width'] - $config['width']) / 2;
			$config['y_axis'] = $offset > 0 ? $offset : ($imgData['height'] - $config['height']) / 2;
